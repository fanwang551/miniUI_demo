...
namespace Plusoft.Web
{
    public class TreeService : BaseService
    {
        ...
        public void LoadTree()
        {
            ...
            ArrayList folders = DapperHelper.Query(sql);
        ...
        public void LoadNodes2()
        {
            ...
            for (int i = 0, l = folders.Count; i < l; i++)
            {
                ...
                ArrayList nodes = DapperHelper.Query(sql2);
        ...
        public void SaveTree()
        {
            ...
            for (int i = 0, l = list.Count; i < l; i++)
            {
                ...
                if (state == "added")
                {
                    ...
                    new FileBLL().Insert(node);
                }
                else
                {
                    new FileBLL().Update(node);
                }
            }
            // Remove Node
            if (removed != null)
        ...
        public void FilterTree()
        {
            ...
            //获取整个树数据
            String sql = "select * from plus_file order by num, updatedate";
            ArrayList nodes = DapperHelper.Query(sql);
